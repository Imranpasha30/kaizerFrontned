        ...
        {STEPS.map((label, i) => (
          <React.Fragment key={i}>
            <div className={`flex items-center gap-1.5 text-xs font-medium
              ${i === step ? "text-accent2" : i < step ? "text-green-400" : "text-gray-600"}`}>
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold
                ${i === step ? "bg-accent text-white" : i < step ? "bg-green-800 text-green-300" : "bg-[#1a1a1a] text-gray-600"}`}>
                {i < step ? "✓" : i + 1}
              </div>
      ...
      <div className="card p-6">
        ...
        {step === 0 && (
          <div>
            ...
            <label
              ref={dropRef}
              className="border-2 border-dashed border-border rounded-lg p-10 flex flex-col items-center gap-3 cursor-pointer hover:border-gray-500 transition-colors"
            >
              <input
                type="file"
                accept="video/*"
              ...
              {file ? (
                ...
                <>
                  ...
                  <span className="text-gray-400">Drag & drop or click to select video</span>
            ...
            <div className="flex flex-col gap-3">
              {Object.entries(platforms).map(([key, info]) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 mt-0.5">{info.width}×{info.height}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        {step === 3 && (
          <div>
            ...
            <div className="bg-black/40 rounded-lg p-4 flex flex-col gap-3 mb-4 text-sm">
              <Row label="Video"    value={file?.name} />
              <Row label="Platform" value={platforms[platform]?.label} />
