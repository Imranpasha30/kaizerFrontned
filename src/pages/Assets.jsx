...
function AssetCard({ asset, folders = [], currentFolder = "", onSetDefault, onUnsetDefault, onDelete, onMove }) {
  ...
  // Job badge — runner stamps generated/bulletin assets with a
  // ``"job:<id>"`` tag.  Surfacing it here means the user can tell
  // which job produced which image without us creating a fresh
  // folder per job (which was bloating the folder bar to dozens of
  // tabs).  Also detect the "generated"/"bulletin" tags so we can
  // show a small "AI" badge for OpenAI-origin images.
  const tags = Array.isArray(asset.tags) ? asset.tags : [];
  const jobTag = tags.find((t) => typeof t === "string" && t.startsWith("job:"));
  const jobId  = jobTag ? jobTag.slice(4) : "";
  const isGenerated = tags.includes("generated") || tags.includes("bulletin");

  // Download = use the asset's primary URL.  We avoid window.open so a
  // popup blocker can't kill it; a hidden <a download> click streams
  // the file via the browser's built-in download handler.
  function handleDownload(e) {
    e.preventDefault();
    e.stopPropagation();
    const url = api.mediaUrl(asset.url || asset.thumb_url);
    if (!url) return;
    const a = document.createElement("a");
    a.href = url;
    a.download = asset.filename || `asset_${asset.id}`;
    // Cross-origin (R2) needs target=_blank so the browser doesn't
    // try to navigate; same-origin (/media/...) downloads inline.
  ...
  return (
    ...
    >
      <div className="relative aspect-square bg-black">
        ...
        {/* Corner badges — non-overlapping. Default badge top-left,
            AI/job badges top-right. */}
        {asset.is_default_ad && (
        ...
        {(isGenerated || jobId) && (
          <div className="absolute top-1.5 right-1.5 flex flex-col gap-1 items-end">
            {isGenerated && (
              <span
                className="bg-accent2/90 text-white text-[10px] px-1.5 py-0.5 rounded font-medium flex items-center gap-0.5"
                title="Generated by OpenAI for a full video"
              >
                <Sparkles size={9} /> AI
              </span>
            )}
            {jobId && (
              <span
                className="bg-black/70 text-gray-200 text-[10px] px-1.5 py-0.5 rounded font-medium"
                title={`Generated for job #${jobId}`}
              >
                job #{jobId}
              </span>
            )}
        ...
        <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-1.5 p-2">
          <button
            ...
            className="w-full px-2 py-1.5 rounded bg-accent2/80 hover:bg-accent2 text-white text-xs flex items-center justify-center gap-1.5"
